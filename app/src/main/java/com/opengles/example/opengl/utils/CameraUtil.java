...
public class CameraUtil {
    ...
    public void initCamera(int width, int height) {
        ...
        Camera.Size pictureSize = getOptimalPictureSize(640,480);
        //设置照片尺寸
        parameters.setPictureSize(pictureSize.width, pictureSize.height);
        //设置实时对焦 判断一下 部分手机不支持会crash
    ...
    /**
     * 通过传入的宽高  计算出最接近相机照片支持的宽高
     *
     * @param w
     * @param h
     * @return 返回一个Camera.Size类型 通过setPictureSize设置给相机
     */
    public Camera.Size getOptimalPictureSize(int w, int h) {
    ...
    public void takePicture(String fileName, String filePath) {
        ...
        // TODO: 18-6-18 如果想要在相册里看见该图片 需要更新系统图库，这里没做处理
            ...
            public void onPictureTaken(byte[] data, Camera camera) {
                ...
                try {
                    if (mPicListener != null) {
                        ...
                        Camera.CameraInfo info = new Camera.CameraInfo();
                        Camera.getCameraInfo(mCameraId, info);
                        realBmp = FileUtil.rotaingBitmap(info.orientation, bitmap);

                        ByteArrayOutputStream baos = new ByteArrayOutputStream();
                        realBmp.compress(Bitmap.CompressFormat.JPEG, 100, baos);
                        File file = FileUtil.saveFile(realBmp, mFileName, mFileDir + "/");
                    ...
                    if (mPicListener != null) {
                        mPicListener.onPictureTaken("保存失败：" + e.getMessage(), null);
                    }
                }
                if (mCamera != null){
                    mCamera.startPreview();
